...
$MAX_MEMORY_MB = 1024
$FOUNDRY_SERVER_IP = "127.0.0.1"
$FOUNDRY_SERVER_PORT = "30000"
...
$RESERVED_SHARE = (($zid -replace '[^a-zA-Z0-9]', '') + "foundryvtt").ToLower()
...
if ($targetEnvironment) {
    ...
    if ($shares) {
        ...
        Invoke-Expression "$PATH_TO_ZROK reserve private ${FOUNDRY_SERVER_IP}:${FOUNDRY_SERVER_PORT} --backend-mode tcpTunnel --unique-name $RESERVED_SHARE"
    }
} else {
